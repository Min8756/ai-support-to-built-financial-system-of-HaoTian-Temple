...
if current_role == "admin":
    ...
    st.markdown("根据审计合规要求，请在此处将固定账户（财务、当家）精准绑定到具体负责人的**姓名、大陆身份证、手机号**。")
    
    for u_key in ["finance", "temple_head"]:
        u_data = st.session_state.user_db[u_key]
        with st.expander(f"⚙️ 更改与管理：{u_data['title']} ({u_key})"):
            edit_pwd = st.text_input(f"[{u_data['title']}] 登录密码修改", value=u_data["password"], key=f"pwd_{u_key}")
            edit_name = st.text_input(f"[{u_data['title']}] 绑定负责人姓名", value=u_data["name"], key=f"name_{u_key}")
            edit_phone = st.text_input(f"[{u_data['title']}] 负责人绑定手机号", value=u_data["phone"], key=f"phone_{u_key}")
            edit_id = st.text_input(f"[{u_data['title']}] 负责人大陆身份证号", value=u_data["id_card"], key=f"id_{u_key}")
            
            if st.button(f"保存 {u_data
